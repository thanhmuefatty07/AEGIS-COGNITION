...
Maturin is the root PEP 517 backend, targeting
`aegis_cognition.aegis_nerve`; Cargo enables `python-extension` only for wheel
builds. `uv.lock` is the environment authority and tests run through `uv run`.
