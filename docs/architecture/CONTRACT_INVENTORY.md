...
| `AcceleratorExecutor` | Rust execution seam | Vendor adapters are optional; exact healthy profile identity and capability matching precede execution; closure compatibility helper is accounting-only | `PROVEN` request-gate and lane-accounting tests; real vendor/iGPU backend execution `NOT VERIFIED` |
