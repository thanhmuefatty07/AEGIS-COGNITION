...
def nested_mirror():
    ...
    for p in sorted(root.rglob("*")):
        ...
        if not sout.strip():
            _, sout, _=git("status","--porcelain","--","docs/archive/legacy")
        refs=[]
        for dpath, dnames, fnames in os.walk(ROOT):
            dnames[:]=[d for d in dnames if d not in {".git",".local",".venv","target",".mypy_cache",".pytest_cache",".ruff_cache",".serena","artifacts",".agents",".cursor",".sixth","brain","planning pdf"}]
            for fn in fnames:
                q=Path(dpath)/fn
        ...
        for dpath,dnames,fnames in os.walk(ROOT):
            dnames[:]=[d for d in dnames if d not in {".git",".local",".venv","target",".mypy_cache",".pytest_cache",".ruff_cache",".serena","artifacts",".agents",".cursor",".sixth","brain","planning pdf","evidence-pack"}]
            for fn in fnames:
                q=Path(dpath)/fn
    ...
    return {"status":"INSPECTED","root":root.relative_to(ROOT).as_posix(),"files":rows,"tree_classification":"ARCHIVE_ONLY","conclusion":"Historical files relocated to docs/archive/legacy; inspect per-file references and history before considering removal","limitations":["search is bounded to current checkout and excludes generated/cache trees"]}
