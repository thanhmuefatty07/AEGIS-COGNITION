...
command, commit, dirty-worktree status and SHA-256 fingerprint, timestamps,
duration, platform, toolchain, counts, output hashes, and explicit
local/independent-verification scope into the
...
3. Suite counts always include command, commit, worktree status/fingerprint, timestamps, duration, platform, toolchain, discovered, passed, failed, ignored, filtered, output hashes, and claim scope. A local `PROVEN` status is labeled `LOCALLY PROVEN` and remains `INDEPENDENTLY NOT VERIFIED`.
