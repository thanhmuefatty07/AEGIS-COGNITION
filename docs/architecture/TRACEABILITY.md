...
| PY-001 | Production/forward Python lanes | `.python-version`, `.github/workflows/ci.yml` | `PROVEN`: required 3.14.7 / 3.15.0-rc.1 plus experimental free-threaded lanes passed CI run [32778221183](https://github.com/thanhmuefatty07/AEGIS-COGNITION/actions/runs/32778221183) |
| PKG-001 | Maturin owns the native wheel build | `pyproject.toml`, Cargo `python-extension` feature | `LOCALLY PROVEN` pinned 1.14.1 configuration and local clean-wheel import on the observed Windows CPython lane; exact Tier-1 matrix and independent verification remain `NOT VERIFIED` |
| RUST-001 | Rust 2024 migration | all workspace manifests and Windows FFI declarations | `PROVEN` workspace library check passes |
| WASM-001 | Wasmtime 47 migration is security-reviewed | `Cargo.toml`, `WASMTIME_MIGRATION.md`, sandbox tests, `fuzz/` | `PROVEN` exact 47.0.3 configuration/static gates, Rust behavior tests, and resource-contract fuzz smoke in deep run [32778232964](https://github.com/thanhmuefatty07/AEGIS-COGNITION/actions/runs/32778232964); broader Wasmtime/adversarial and hostile-kernel evidence `NOT VERIFIED` |
| CI-001 | CI runs the resolved environment | `.github/workflows/ci.yml`, `.github/workflows/deep.yml` | `PROVEN`: pinned actions, locked uv, strict checks, 10/10 fast CI jobs in run [32778221183](https://github.com/thanhmuefatty07/AEGIS-COGNITION/actions/runs/32778221183), and 3/3 deep jobs in run [32778232964](https://github.com/thanhmuefatty07/AEGIS-COGNITION/actions/runs/32778232964) |
| DEEP-001 | Security, replay, and dependency evidence is retained | `.github/workflows/deep.yml`, `docs/architecture/VERIFICATION_BASELINE.md` | `SOURCE-BACKED HISTORICAL`: the cited hosted run is not evidence for the current local final SHA; current fuzz/Miri/AddressSanitizer and hosted-runner observation remain `NOT VERIFIED` |
...
| TEL-002 | Python provider/tool/evidence events share Rust-compatible correlation IDs | `aegis_cognition/observability.py`, `core/python/aegis_adapter.py`, `core/rust/src/ffi.rs` | `LOCALLY PROVEN` by bounded-chain tests; native wheel FFI forwarding and independent verification remain `NOT VERIFIED` |
...
| TOOL-002 | Fast CI excludes POCs while deep evidence retains them | `Cargo.toml`, `ci.yml`, `deep.yml` | `PROVEN`: configuration, 10/10 CI result, 3/3 deep result, and plugin workflow pass [32778221299](https://github.com/thanhmuefatty07/AEGIS-COGNITION/actions/runs/32778221299) |
