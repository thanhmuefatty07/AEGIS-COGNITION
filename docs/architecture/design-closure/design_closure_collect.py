...
def fixed_reports(base, ep):
    ...
    retry={"schema":"aegis-design-retry-truth-v1","layers":[
      ...
      {"owner":"provider SDK","trigger":"SDK transient","default":"UNKNOWN","max_attempts":"UNKNOWN","backoff":"UNKNOWN","timeout":"UNKNOWN","effect_class":"provider external","attempt_id":"UNKNOWN","budget_integration":"UNKNOWN","idempotency":"provider-specific","cancellation":"provider-specific","formula":"N_sdk UNKNOWN","evidence":"provider package not contacted"}],"derived_bounds":["non-Lab AegisAgent over provider route: N_provider_invocations <= 3*P before unknown SDK retries","Lab gateway uses AegisAdapter (not AegisAgent): N_provider_invocations <= G*P before unknown SDK retries","Lab gateway/tool/experiment/simulation local receipts have deterministic idempotency keys and finite cooperative deadlines","ProcessExecutionCell itself performs no retry: N_process_cell <= 1 per invocation; an enclosing Lab policy may still retry the admission","global N_external is NOT VERIFIED because user runners/callables, external effects and SDK retry semantics remain open"],"duplicate_effect_risks":["outer Lab gateway retries any Exception, including ambiguous timeout after external commit","adapter/provider/SDK layers can multiply attempts","provider candidate fallback may execute a different external effect","user-supplied runner can contain its own retry","non-cooperative or synchronous runner can outlive an asyncio deadline"],"classification":"LAYERED_RETRY_OWNER_OPEN","limitations":["no live provider or SDK retry probe"]}
    ...
    conf={"schema":"aegis-design-config-precedence-v1","precedence_by_boundary":[
      ...
      {"boundary":"retry counts","order":"per-run options override defaults, then cap at max_steps; no provider SDK override known","evidence":"lab.py gateway/tool/experiment/simulation"},
