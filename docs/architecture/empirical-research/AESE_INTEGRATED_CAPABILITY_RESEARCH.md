...
`VERIFIED` by current Compute Engine inspection: the project currently contains two `e2-standard-2` instances, `aegis-test-linux-01` in `asia-southeast1-a` and `aegis-test-linux-02` in `asia-southeast1-b`; both are `TERMINATED`, each retains a 30 GB `pd-standard` boot disk with `autoDelete=true`, and neither currently has a named static address or snapshot listed. The controlled probe in this research used only `aegis-test-linux-02`; `aegis-test-linux-01` was not started or modified. `SOURCE-BACKED` by Google Cloud billing documentation: stopping removes CPU-instance charges but attached disks and other retained resources continue to incur charges; an alerts-only budget does not automatically cap spending. Therefore “VM stopped” is not equivalent to “zero cloud cost”, and the project currently retains at least 60 GB of persistent disk capacity. Any future run record must check both instance state and retained resources, set a TTL/cost ceiling before start, verify termination after cancellation, and report the remaining disk/resource cost state. Deleting either current instance or disk is intentionally out of scope until the owner explicitly requests it and data preservation is confirmed. See [stop/terminate billing behavior](https://docs.cloud.google.com/compute/docs/instances/suspend-stop-reset-instances-overview), [disk and image pricing](https://cloud.google.com/compute/disks-image-pricing), and [Cloud Billing budgets](https://docs.cloud.google.com/billing/docs/how-to/budgets).
