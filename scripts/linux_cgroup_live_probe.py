...
def probe(output: Path | None = None, *, cgroup_root: Path = Path("/sys/fs/cgroup")) -> dict[str, object]:
    report: dict[str, object] = {
        ...
        "status": "NOT VERIFIED",
        "checks": {},
        "limits": {"memory_max_bytes": 32 * 1024 * 1024, "cpu_max": "10000 100000", "pids_max": 4},
        "verification_scope": {
            "verified": [],
    ...
    required = ["cgroup.controllers", "cgroup.procs", "memory.max", "memory.current", "memory.events", "cpu.max", "pids.max"]
    ...
    try:
        ...
        required_checks = ["limits_configured", "child_attached", "live_memory_accounting_observed", "cgroup_kill_termination_observed"]
        if all(checks.get(name) is True for name in required_checks):
            ...
            if not checks.get("memory_pressure_oom_kill_observed"):
